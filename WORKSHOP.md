...
That's a **26x reduction** in input tokens per request. The savings compound: cheaper model *and* fewer tokens.
...
At 1M requests/day, the fine-tuned model costs **$76/month** vs **$35K/month** for the frontier model — a **~467x cost reduction**.
...
```json
{"messages": [{"role": "user", "content": "Bonjour, comment allez-vous?"}, {"role": "assistant", "content": "fr"}]}
```
...
| Document type classification (vision) | Visual taxonomy, examples of each doc type, distinguishing features | *(scanned page image)* | `w2_form` |
